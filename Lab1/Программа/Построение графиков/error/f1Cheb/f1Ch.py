...
x10 = []
y10 = []
with open('cheb1_5.txt', 'r') as file:
    for line in file:
        x, y = map(float, line.split(':'))
        x5.append(x)
        y5.append(y)

with open('cheb1_7.txt', 'r') as file:
    for line in file:
        x, y = map(float, line.split(':'))
        x7.append(x)
        y7.append(y)

with open('cheb1_10.txt', 'r') as file:
    for line in file:
        x, y = map(float, line.split(':'))
        x10.append(x)
        y10.append(y)
...
plt.xlabel('x')
plt.ylabel('y')
plt.title('поточечная ошибка на функции(1) на Чебышевской сетки')
